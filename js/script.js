    ...
    kmlDoc.querySelectorAll('Placemark').forEach(placemark => {
        ...
        function parseAndAddPoint(pointElement, date, position, descriptionUrl, iconGetter = getPointIcon) {
            ...
            if (iconGetter === getMilEquipIcon) {
                ...
                // Сохраняем маркер и его категорию
                window.allEquipmentMarkers.push({ marker: marker, category: category });
                // Можно также прикрепить категорию к самому маркеру
...
function updateEquipmentFilter() {
    ...
    if (selectAll.checked) {
        // Если выбран "Все", то фильтр null (все категории)
        window.selectedEquipmentCategories = null;
        // Убедимся, что все чекбоксы отмечены
        catCheckboxes.forEach(cb => cb.checked = true);
    } else {
        // Иначе фильтр — массив выбранных категорий
        window.selectedEquipmentCategories = selected.length > 0 ? selected : [];
    }
    
    // Если ничего не выбрано — техника невидима и кнопка неактивна
    const milEquipBtn = document.getElementById('mil-equip-btn');
...
function toggleEquipmentMenu() {
    ...
    if (!isVisible) {
        ...
        if (window.allEquipmentMarkers.length === 0) {
            // Загружаем слой техники (функция уже существует)
            initMilequipLayer(window.milequipKmlPaths).then(() => {
                // После загрузки устанавливаем фильтр "Все" и показываем
                document.getElementById('equip-select-all').checked = true;
                updateEquipmentFilter();
            });
        } else if (!window.isMilEquipVisible) {
            // Если техника была скрыта, включаем её с текущим фильтром
            window.isMilEquipVisible = true;
            applyEquipmentFilter();
        }
    } else {
